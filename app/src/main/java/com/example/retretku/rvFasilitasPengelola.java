...
public class rvFasilitasPengelola extends RecyclerView.Adapter<rvFasilitasPengelola.LFPViewHolder> {
    ...
    public void onBindViewHolder(@NonNull LFPViewHolder holder, int position) {
        holder.nama.setText(facilities.get(position).getNama_fasilitas());

        String tempTextHolder = "Qty: " + facilities.get(position).getJumlah_fasilitas();
        holder.jml.setText(tempTextHolder);

        tempTextHolder = "x Rp. " + facilities.get(position).getHarga_fasilitas();
        holder.hrg.setText(tempTextHolder);

        holder.desc.setText(facilities.get(position).getDeskripsi_fasilitas());
    }
